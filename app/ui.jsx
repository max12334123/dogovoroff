...
export const EASE=[.22,.61,.21,1];
export const CONFIG={brand:"ДоговорОфф",phone:"+7 (3466) 000-00-00",phoneHref:"tel:+73466000000",email:"dogovor.off@mail.ru",web3formsKey:"109ee3ca-96c1-4a49-8dd9-9c3611d26b16",telegram:"https://t.me/dogovor_off",max:"https://max.ru/",address:"г. Нижневартовск, ул. Ленина, 6, офис 402",hours:"Пн–Пт 09:00–19:00 · заявки — 24/7",geo:"61°32′ N — 76°58′ E"};
export const CSS=`
...
export function Logo({className,style}){
  return(
    <svg className={className} style={style} viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="square" aria-hidden="true">
      <path d="M16 34 L19 11 H31 V34"/>
      <path d="M9 38 H39"/>
      <path d="M9 38 L6.5 44 M39 38 L41.5 44"/>
      <path d="M25 17 l2.1 4.9 4.9 2.1 -4.9 2.1 -2.1 4.9 -2.1 -4.9 -4.9 -2.1 4.9 -2.1 z" fill="currentColor" stroke="none"/>
    </svg>
  );
}
export function Reveal({children,delay=0,x=0,scale=1,className="",style}){
  return(<motion.div className={className} style={style}
    initial={{opacity:0,y:30,x,scale,filter:"blur(8px)"}}
    whileInView={{opacity:1,y:0,x:0,scale:1,filter:"blur(0px)"}}
    viewport={{once:true,margin:"-70px"}}
    transition={{duration:1,ease:EASE,delay}}>{children}</motion.div>);
}
export function Counter({to,suf="",duration=2}){
...
export function useAnimatedNumber(t){
...
export function Magnetic({href,className,children,onClick,type}){
  const ref=useRef(null);
  ...
  return(<Tag ref={ref} href={href} type={type} onClick={onClick} className={className} style={{x:sx,y:sy}} whileTap={{scale:.97}} onMouseMove={move} onMouseLeave={leave}>{children}</Tag>);
}
export function Cursor(){
  ...
  return<>
    ...
    <motion.div className={"cursor-ring"+(hover?" on":"")} style={{x:rx,y:ry}} animate={{scale:hover?1.7:1}} transition={{duration:.35,ease:EASE}}/>
  </>;
}
...
export const MARQUEE=["Тендеры и госзакупки","ФАС","Арбитраж","Юраутсорсинг","ЖКХ и УК","Договоры","Претензии","Банкротство"];
