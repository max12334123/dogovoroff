...
begin
  ...
  left join lateral (
    ...
    join public.organization_members as assignee_membership
      on assignee_membership.organization_id = m.organization_id
      and assignee_membership.user_id = mp.user_id
      and assignee_membership.role in ('admin', 'lawyer')
...
comment on function public.list_staff_matter_assignments(uuid[]) is
  'Read-only assignment summary for same-organization staff who may manage each requested matter. Reports only assignees with current membership in that matter organization.';
